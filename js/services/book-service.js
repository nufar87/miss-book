...
function getBookById(id) {
  console.log(id);
  return Promise.resolve(gBooks.find((book) => book.id === id));
}
...
// function addReview(bookId, Userreview) {
...
function getNextBook(bookId) {
  ...
  console.log(index);
  if (index === gBooks.length - 1) return;
  index++;
  return Promise.resolve(gBooks[index].id);
}
...
//   return get(bookId).then((book) => {
//     var currReviewIdx = book.reviews.findIndex(
